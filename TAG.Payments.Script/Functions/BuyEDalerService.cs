...
{
	/// <summary>
	/// Defines a service for buying eDaler.
	/// </summary>
	public class BuyEDalerService : ServiceDefinition
	{
		/// <summary>
		/// Defines a service for buying eDaler.
		/// </summary>
		...
		public BuyEDalerService(ScriptNode Argument, int Start, int Length, Expression Expression)
		...
		/// Name of function.
		/// </summary>
		public override string FunctionName => nameof(BuyEDalerService);
		...
		public override IElement Evaluate(Dictionary<string, IElement> Definition, Variables Variables)
		{
			...
			ILambdaExpression CanBuyEDaler = null;
			ILambdaExpression GetOptions = null;
			ILambdaExpression BuyEDaler = null;
			...
			foreach (KeyValuePair<string, IElement> P in Definition)
			{
				switch (P.Key.ToLower())
				{
					...
					case "canbuy":
						...
						{
							if (L2.NrArguments != 1)
								throw new ScriptRuntimeException("CanBuyEDaler method must only have one argument.", this);
						}
						else
						{
					...
					case "buy":
						...
						{
							if (L4.NrArguments != 1)
								throw new ScriptRuntimeException("BuyEDaler method must only have one argument.", this);
						}
						else
							throw new ScriptRuntimeException("BuyEDaler method must be a lambda expression.", this);
			...
			if (BuyEDaler is null)
				throw new ScriptRuntimeException("BuyEDaler method missing.", this);

			IBuyEDalerService Service = new Providers.BuyEDaler.BuyEDalerService(Id, Name, IconUrl, IconWidth, IconHeight,
				TemplateContractId, Supports, CanBuyEDaler, GetOptions, BuyEDaler, Variables);

			Providers.BuyEDaler.BuyEDalerServiceProvider.Register(Service);
